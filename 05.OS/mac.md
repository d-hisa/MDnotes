...
## Trouble Shoot
スリープからふっきしなくなることがあったりしたので試す。
### Safe boot
セーフブートし、キャッシュをクリアする。
- 電源を切る
- <kbd>Shift</kbd>を押しながら電源を入れる
- ログイン画面の右上に **セーフブート** と表示されていることを確認し、ログインする
- ログイン後、`このMacについて`->`概要`->`システムレポート`->`ソフトウェア`->`起動モード`が **セーフ** であることを確認
- 再起動

### 通信のスリープ設定を解除
- `システム環境設定`->`省エネルギー`->`Wi-Fiネットワークアクセスによるスリープ解除`をDisableする

### SMCリセット
- システム終了
- `Shift`+`Ctrl`+`option`を押しながら電源ボタンを押し、10秒間押し続ける
- 電源ボタンを押し、電源を入れる

### PRAMリセット
- システム終了
- 起動し、即座に`option`+`cmmand`+`P`+`R`を20秒押す
- macの起動が終われば、適宜リセットされた設定を再度設定する
